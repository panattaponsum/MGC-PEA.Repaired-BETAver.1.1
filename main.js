...
window.saveData = async function() {
    ...
    // 💡 NEW & MODIFIED: ตรวจสอบเงื่อนไขทั้งหมดสำหรับการเพิ่มรายการใหม่เท่านั้น (editIndex < 0)
    if (editIndex < 0) { 
        const latestRecord = records.length > 0 ? records[records.length - 1] : null;
        const currentStatus = latestRecord ? latestRecord.status : 'ok'; // สถานะปัจจุบัน

        // 1. ป้องกันการบันทึก 'ชำรุด' ซ้ำซ้อน
        if (statusVal === 'down' && currentStatus === 'down') {
            Swal.fire({
                title: 'ไม่สามารถบันทึกได้',
                text: `อุปกรณ์ "${currentDevice}" มีสถานะ "ชำรุด" อยู่แล้ว. หากต้องการบันทึกการซ่อมแซม กรุณาปิดรายการชำรุดปัจจุบัน โดยการแก้ไขรายการล่าสุดเป็น "ใช้งานได้" ก่อนบันทึกข้อมูลใหม่ `,
                icon: 'warning'
            });
            return false;
        }
        
        // 2. ป้องกันการบันทึก 'ใช้งานได้' (ok) ใหม่ ที่มี BrokenDate/FixedDate ในขณะที่อุปกรณ์ชำรุดอยู่
        // นี่คือเงื่อนไขที่คุณต้องการ: ห้ามสร้าง "รายการซ่อมเสร็จ" ใหม่ หากรายการ "ชำรุด" ค้างอยู่
        if (statusVal === 'ok') {
            if (currentStatus === 'down') {
                // ถ้าอุปกรณ์ชำรุดอยู่ และผู้ใช้บันทึกเป็น 'ok'
                // ระบบจะไม่บล็อก, แต่จะอนุญาตให้รายการใหม่นี้ 'ปิด' สถานะลง
                // ***หมายเหตุ: การแก้ไขครั้งที่แล้ว (การบล็อกเมื่อ currentStatus === 'ok' และ statusVal === 'ok') 
                //            เป็นตรรกะที่เข้มงวดเกินไป, ควรอนุญาตให้บันทึก 'ok' ซ้ำได้ หากต้องการอัปเดตสถานะเฉยๆ***
                
                // ***ดังนั้น จะไม่มีการบล็อกตรงนี้ หากต้องการปิดสถานะ 'down'***

            } else { // currentStatus === 'ok'
                // ถ้าอุปกรณ์ใช้งานได้อยู่แล้ว และผู้ใช้บันทึกเป็น 'ok' (ใช้งานได้)
                // ห้ามมีวันที่ชำรุด/ซ่อมแซม เพราะจะเป็นการบันทึกประวัติซ้ำซ้อน
                 if (brokenDate || fixedDate) {
                    Swal.fire({
                        title: 'ข้อมูลขัดแย้ง',
                        text: 'อุปกรณ์อยู่ในสถานะ "ใช้งานได้" อยู่แล้ว. การบันทึก "ใช้งานได้" ซ้ำซ้อน กรุณาแก้ไขรายการล่าสุดเพื่อปิดสถานะชำรุดเดิมก่อนสร้างรายการใหม่',
                        icon: 'warning'
                    });
                    return false;
                }
